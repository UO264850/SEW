class FileManager {
    ...
    subirGeoJson() {
        var ficheroProcesado = document.getElementsByName("subirGeoJson")[0].files[0];
        var lugares = $("Geometry", ficheroProcesado);
        for(var i =0; i<lugares.length; i++){
            var coord = $("coordinates", lugares[i]).value().split(",");
            var lat = coord[0];
            var lon = coord[1];
